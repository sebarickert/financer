...
describe("Basic account flow", () => {
  ...
  it('Verify accounts in fixture', function () {
    cy.get('[data-test-id="account-row"').should("have.length", 6)
  });

  it('Add new account', function () {
    cy.get('[data-test-id="account-row"').should("not.have.text", "New Test Account")
    ...
    expect(cy.get('[data-test-id="balance"] > dd').contains("1 000,00 €")).to.exist 
